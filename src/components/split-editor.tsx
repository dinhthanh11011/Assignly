...
/** Xem trước phần mỗi người phải chịu — dùng đúng phép chia của server. */
...
/**
 * Chọn người trả và cách chia một giao dịch giữa các thành viên. Chỉ nên hiện khi
 * sổ có từ 2 người trở lên — sổ một người thì mặc định "chia đều" là đủ.
 */
export function SplitEditor({
  ...
  return (
    <div className="space-y-4 rounded-xl border border-hairline bg-sunken p-3.5">
      <div className="space-y-2">
        <Label>{type === "INCOME" ? "Ai nhận tiền?" : "Ai trả tiền?"}</Label>
        <div className="no-scrollbar -mx-1 flex gap-1.5 overflow-x-auto px-1 py-0.5">
          {members.map((m) => {
            ...
            return (
              ...
              >
                <MemberAvatar user={m} className="size-6" />
      ...
      <div className="space-y-2">
        ...
        <div className="divide-y divide-border/50 overflow-hidden rounded-lg border border-hairline bg-card">
          {members.map((m) => {
            ...
            return (
              <div
                key={m.id}
                className={cn(
                  "flex items-center gap-2.5 px-2.5 py-2 transition-opacity",
                  !on && "opacity-45"
                )}
              >
                {value.mode === "EQUAL" ? (
                  ...
                  >
                    <MemberAvatar user={m} className="size-7" />
                    ...
                    {value.mode === "WEIGHT" ? (
                      <>
                        <Stepper
                          ...
                          label={memberLabel(m)}
                        />
                        <span className="num w-20 shrink-0 text-right text-xs font-bold">
                          {formatMoney(share)}
                        </span>
                      </>
                    ) : (
...
function Stepper({
  ...
  const btn =
    "flex size-7 items-center justify-center rounded-full bg-sunken text-muted-foreground transition-colors hover:text-primary disabled:opacity-40";
  return (
    <div className="flex shrink-0 items-center gap-1">
      ...
      <span className="num w-5 text-center text-sm font-bold">{value}</span>
      <button
        type="button"
        className={btn}
